...
def get_tight_bounding_box(mask, class_id, h, w):
    """
    Get a normalized tight oriented bounding box for the given mask.
    ...
    Returns:
    list: A list containing the class_id and the normalized coordinates
          (x1, y1, x2, y2, x3, y3, x4, y4) of the tight bounding box.
    """
    ...
    box_normalized = [(point[0] / w, point[1] / h) for point in box]

    # Flatten the box points and prepend the class_id
    box_flat = [coord for point in box_normalized for coord in point]
    return [class_id] + box_flat
